...
__global__ void mykernel(int input_size) {
  ...
  vm_init(&vm, data, storage, pt, &pagefault_num, PAGE_SIZE,
          ...
          PHYSICAL_MEM_SIZE / PAGE_SIZE, threadIdx.x, results);

  // user program the access pattern for testing paging
  user_program(&vm, input, vm.results, input_size);
  __syncthreads();
}
...
int main() {
  ...
  mykernel<<<1, 4, INVERT_PAGE_TABLE_SIZE>>>(input_size); // 1 blocks with 4 threads
